...
def _read(reghi, reglo, n, delay_s=0.01):
    reg_buf[0] = reghi
    reg_buf[1] = reglo
    i2c.writeto(_ADDR, reg_buf)
    time.sleep(delay_s)
    return i2c.readfrom(_ADDR, n)

def _write(reghi, reglo, cmd):
    reg_buf[0] = reghi
    reg_buf[1] = reglo
    #print("sswrite: ", [hex(i) for i in reg_buf+cmd])
    i2c.writeto(_ADDR, reg_buf+cmd)
...
def init():
    ...
    while not _ADDR in i2c.scan():
        attempts += 1
        if attempts > 5:  # after 5 tries, give up
            raise OSError("Seesaw device not found")
        print("Crickit not found! Retrying...")
        time.sleep(0.1)
    reg_buf[0] = 0x7F
    reg_buf[1] = 0xFF
    i2c.writeto(_ADDR, reg_buf)
...
def encoder_position(encoder=0):
    return struct.unpack(">i", _read(0x11, 0x00 + encoder * 0x10, 4))[0]

def encoder_enable_interrupt(encoder=0, enable=True):
    val = b'\x01' if enable else b'\x00'
    _write(0x11, 0x10 + encoder * 0x10, val)

def encoder_delta(encoder=0):
    return struct.unpack(">i", _read(0x11, 0x04 + encoder * 0x10, 4))[0]

def encoder_set_mode(encoder=0, mode=0x00):
    """Set encoder count mode (e.g. 0x00 = default, 0x01 = double-count)."""
    _write(0x11, 0x0E + encoder * 0x10, bytes([mode]))
