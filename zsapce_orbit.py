...
def simulate_trajectory(index, steps=25, dt=10.0):
    sim_pos = list(player['pos'])
    sim_vel = list(player['vel'])
    sim_mass = player['mass']

    sim_planets = [{'pos': list(p['pos']), 'vel': list(p['vel']), 'mass': p['mass']} for p in planets]
    forces = [[0.0, 0.0] for _ in sim_planets]
    ref = sim_planets[index]
    ref_x, ref_y = ref['pos']

    points = [(sim_pos[0] - ref_x, sim_pos[1] - ref_y)]
    ...
    for _ in range(steps):
        ...
        for p in sim_planets:
            ...
            f = G * sim_mass * p['mass'] * inv_dist * inv_dist * inv_dist
            fx += f * dx
            fy += f * dy

        sim_vel[0] += fx * dt / sim_mass
        sim_vel[1] += fy * dt / sim_mass
        sim_pos[0] += sim_vel[0] * dt
        sim_pos[1] += sim_vel[1] * dt

        for i in range(len(sim_planets)):
            forces[i][0] = forces[i][1] = 0.0

        for i in range(len(sim_planets)):
            a = sim_planets[i]
            for j in range(i + 1, len(sim_planets)):
                b = sim_planets[j]
                dx = b['pos'][0] - a['pos'][0]
                dy = b['pos'][1] - a['pos'][1]
                dist_sq = dx * dx + dy * dy
                if dist_sq < 1e-10:
                    continue
                inv_dist = 1.0 / math.sqrt(dist_sq)
                f = G * a['mass'] * b['mass'] * inv_dist * inv_dist * inv_dist
                fx = f * dx
                fy = f * dy
                forces[i][0] += fx
                forces[i][1] += fy
        ...
        for i, p in enumerate(sim_planets):
            inv_mass = 1.0 / p['mass']
            p['vel'][0] += forces[i][0] * dt * inv_mass
            p['vel'][1] += forces[i][1] * dt * inv_mass
            p['pos'][0] += p['vel'][0] * dt
            p['pos'][1] += p['vel'][1] * dt

        target = sim_planets[index]
        points.append((sim_pos[0] - target['pos'][0], sim_pos[1] - target['pos'][1]))
